...
addParameter(p,'CENTER_ROBOT',true);
parse(p,varargin{:});
CENTER_ROBOT = p.Results.CENTER_ROBOT;

% Parse URDF
% urdf_path = '/home/xiyana/Desktop/retargeting/yart/urdf/coman/coman_urdf.xml';
urdf_path = '/home/xiyana/Desktop/retargeting/yart/urdf/reachy/reachy.urdf';
model_name = 'reachy';
% model_name = 'coman';
...
for i_idx = 1:chain.n_link
    ...
    try
        ...
        % disp(ext)
        ext = '.stl';
        mesh_path = ['/home/xiyana/Desktop/retargeting/yart/urdf/reachy/visual/',name,ext]; 
        % if exist(mesh_path,'file')
        %    disp(['The mesh path EXISTS: ', mesh_path]);
        % else
        %    disp(['The mesh path DOES NOT EXISTS: ', mesh_path]);
        % end
...
if CENTER_ROBOT
    disp('----------------------------------------------------------');
    %disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs))
    %joint_i = robot.joint{chain.joint(get_topmost_idx(chain)).childs};
    %disp(joint_i.origin.Attributes.xyz);

    %disp(class(chain.joint(chain.joint(get_topmost_idx(chain)).childs).p_offset))
    %disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs).p_offset)

    %disp(class(robot.joint{chain.joint(get_topmost_idx(chain)).childs}));
    %disp(size(robot.joint{chain.joint(get_topmost_idx(chain)).childs}));
    
    %disp(robot.joint{chain.joint(get_topmost_idx(chain)).childs});

    % disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs(1)));
    % disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs(2)).p_offset);
    % disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs(3)));
    % disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs(4)));
    % disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs(5)));
    % disp(chain.joint(chain.joint(get_topmost_idx(chain)).childs(6)));

    %disp(robot.joint{chain.joint(get_topmost_idx(chain)).childs(2)});
    %disp(robot.joint{chain.joint(get_topmost_idx(chain)).childs(2)}.p_offset);

    %p_offset
    %disp(robot);
    %disp(joint_i.child.Attributes.link);
    disp('--------------------------------------------------------------');
    ...
    if isequal(model_name,'reachy') 
       disp("ITS REACHY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
       chain.joint(chain.joint(get_topmost_idx(chain)).childs(5)).p_offset = [0,0,0]';
