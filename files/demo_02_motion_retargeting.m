...
ANIMATE = 1;
SKIP_IF_EXISTS = 0;
model_names = {'reachy'}; % {'social_robot','atlas','baxter','coman'};
...
for i_idx = 1:length(model_names) % for all robot models
    ...
    fprintf('LENGHT OF MOCAP IDXS: %d \n', length(mocap_idxs));
    ...
    for j_idx = 1:length(mocap_idxs) % for all mocap data
        ...
        % Cache path
        cache_path = sprintf('../cache_reachy/simple_motion_retargeting/%s_%s.mat',model_name,mocap_name);
...
for i_idx = 1:length(mocap_idxs) % for different mocap data
    ...
    % Save to Video
    vid_path = sprintf('/home/xiyana/Desktop/cache_reachy/vid/%s_%s.mp4',model_name,mocap_name);
